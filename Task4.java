...
public class Task4 {
	...
	public static class CartesianMapper extends MapReduceBase implements
		...
		public void map(Text key, Text value,
			...
			if (!key.toString().equals(value.toString()) && !key.toString().equals("") && !value.toString().equals("") && (key.toString().compareTo(value.toString()) < 0)  ) {
				//
/*                 System.out.println("Hello!! String one is: " + key  + "\nString 2 is: "+ value+ "============"); */
                String[] first_tokens = key.toString().split(",",-1);
                String[] second_tokens = value.toString().split(",",-1);
/*                 System.out.println("================String one length is is: " + first_tokens.length  + " String 2 is: "+ second_tokens.length+ "============"); */
                outkey.set(first_tokens[0]+","+ second_tokens[0]);
                int one = 0; int sum_one=0;
                int two = 0; int sum_two=0;
                int mul=0;
                for (int i = 1; i < first_tokens.length; i++) {
                    if( !first_tokens[i].equals("") )
                    {
                        one = Integer.parseInt(first_tokens[i]);
                        sum_one=sum_one + (int)Math.pow(one, 2);
                    }
                    else 
                        one=0;
                    
                    if( !second_tokens[i].equals(""))
                    {   
                        two = Integer.parseInt(second_tokens[i]);
                        sum_two=sum_two + (int)Math.pow(two, 2);
                ...
                double sqrt_one = Math.sqrt((double)sum_one);
                double sqrt_two = Math.sqrt((double)sum_two);
                //double total = round ( ((double)mul / (sqrt_one*sqrt_two) ), 2);
                double total =  ((double)mul / (sqrt_one*sqrt_two) );
                String out_this = String.format("%1.2f",total);
                Text output_val = new Text(out_this);
                output.collect(outkey,output_val);
			}
		}
	}
	...
	public static void main(String[] args) throws IOException,
		...
		conf.setJarByClass(Task4.class);
